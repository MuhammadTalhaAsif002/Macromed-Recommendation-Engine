...
def load_data():
    # Main products
    df = pd.read_csv('hybrid_recommended_products.csv')
    df['Has_Image'] = df['Has_Image'].astype(bool)
    df['Dimensions'] = df['Dimensions'].fillna('Not specified')
    df['Image'] = df['Image'].replace('Missing_URL', 'https://via.placeholder.com/247x247?text=No+Image')
    
    # Top products with sales data
    top_df = pd.read_csv('top_products_sales.csv')
    top_df['Image'] = top_df['Image'].fillna('https://via.placeholder.com/247x247?text=No+Image')
...
# Simulate different recommendations based on user type
if user_type == "Hospital Purchaser":
    rec_df = df[df['Price'] > 50].sort_values('Popularity_Score', ascending=False).head(6)
elif user_type == "Surgeon":
    rec_df = df[df['Category'].str.contains('Needle|Scalpel|Scissors')].sort_values('Popularity_Score', ascending=False).head(6)
    ...
    with cols[idx % 3]:
        ...
        <div class="product-card">
            ...
            <div style="position: relative; height: 180px; display: flex; align-items: center; justify-content: center; margin-bottom: 10px;">
                <img src="{product['Image']}" style="max-width: 100%; max-height: 180px; object-fit: contain; border-radius: 5px;">
            </div>
...
for idx, (_, product) in enumerate(top_df.head(10).iterrows()):
    ...
    <div class="product-card product-card-horizontal">
        ...
        <div style="position: relative; height: 120px; display: flex; align-items: center; justify-content: center; margin-bottom: 10px;">
            <img src="{product['Image']}" style="max-width: 100%; max-height: 120px; object-fit: contain; border-radius: 5px;">
        </div>
        <div class="category-tag">{product['Category']}</div>
...
for idx, category in enumerate(categories[:4]):
    with category_cols[idx]:
        category_products = df[df['Category'] == category].sample(1).iloc[0]
        st.markdown(f"""
        <div style="text-align: center; cursor: pointer; padding: 15px; border-radius: 8px; background: white; border: 1px solid #eee; transition: all 0.3s;" 
             ...
             onclick="window.open('{category_products['Title_URL']}', '_blank')">
            <img src="{category_products['Image']}" style="width: 100%; height: 100px; object-fit: contain; margin-bottom: 10px;">
